...
// Garmin's raw exercise-set response for strength training — an undocumented internal
// endpoint (no equivalent in the garmin-connect npm package), the same URL pattern
// used by other community Garmin API wrappers. gc.get() takes a full URL (unlike the
// package's own methods, it doesn't prepend the API host), so we build it from the
// same GC_API base the package uses internally — gc.url is marked private in the
// package's TypeScript types, but that's erased at runtime; the property exists on
// the real object. Wrapped by the caller so a change on Garmin's side degrades
// gracefully instead of breaking the whole activity view.
...
// Best-effort — returns null (rather than throwing) if Garmin's undocumented
// endpoint fails, so the rest of the activity detail view still renders.
