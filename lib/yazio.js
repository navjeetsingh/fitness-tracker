...
/**
 * On-demand historical query for the Discord bot: nutrition summaries for an
 * arbitrary date range, beyond the weekly trend's fixed 7-day window. Unlike
 * getStoredYazioDay, this WILL trigger a live Yazio login for days that were never
 * synced by the nightly cron — acceptable here since it's gated behind the bot's
 * owner-only DM check, not a public endpoint (same reasoning as
 * getGarminHistoricalRange in lib/garmin.js).
 *
 * @param {string} startDateStr - YYYY-MM-DD, inclusive
 * @param {string} endDateStr - YYYY-MM-DD, inclusive
 * @returns {Promise<object[]>} one day summary per date, oldest first
 */
export async function getYazioHistoricalRange(startDateStr, endDateStr) {
  const dates = enumerateDates(startDateStr, endDateStr)
  const days = []
  for (const dateStr of dates) {
    const cached = await getStoredYazioDay(dateStr)
    if (cached) {
      days.push(cached)
      continue
    }
    try {
      days.push(await pullAndStoreYazioSummary(new Date(dateStr + 'T00:00:00Z')))
    } catch {
