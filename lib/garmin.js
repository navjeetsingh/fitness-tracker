...
const STRAVA_MATCH_TOLERANCE_MS = 30 * 60 * 1000 // 30 minutes — generous for GPS-lock delay, tight enough not to cross into a different same-day activity

// Personal records are keyed by Garmin activity ID, but the dashboard's existing
// activity-detail modal only knows Strava IDs — so find that record's Strava
// counterpart by wall-clock start time, not distance: a pace record (fastest 1K/5K/
// etc.) is often just the fastest split *inside* a longer run, not a standalone
// activity near that distance — e.g. a "fastest 1K" can live inside a 10K base run,
// whose total Strava distance is nowhere near 1K. Time is the only reliable anchor.
//
// The one-day window on each side of the target date is required, not just safe
// padding: Strava's start_date_local carries local wall-clock time formatted with a
// trailing 'Z' as if it were UTC, while getStravaActivitiesInRange's after/before
// bounds are true UTC day boundaries. For an athlete ahead of UTC (Bangkok, UTC+7),
// any activity before ~7am local time lands in the *previous* UTC calendar day and a
// same-day-only query misses it outright — confirmed live: a 6:28am local run was
// invisible when queried by its own local date, and only showed up querying the day
// before. Padding both sides keeps this correct for any timezone, not just Bangkok.
async function matchStravaActivity(garminLocalTimestamp) {
  if (!garminLocalTimestamp) return null
  try {
    const target = new Date(garminLocalTimestamp)
    if (isNaN(target)) return null

    const dayStart = new Date(garminLocalTimestamp.split('T')[0] + 'T00:00:00Z')
    ...
    const activities = await getStravaActivitiesInRange(prevDay, nextDay)
    if (!activities?.length) return null

    let best = null
    let bestDiffMs = Infinity
    for (const a of activities) {
      const diffMs = Math.abs(new Date(a.date) - target)
      if (diffMs < bestDiffMs) { bestDiffMs = diffMs; best = a }
    }
    return bestDiffMs <= STRAVA_MATCH_TOLERANCE_MS ? best?.id || null : null
  } catch {
    return null
  }
}
...
async function fetchGarminRecords(gc) {
  ...
  // Strava lookups, only for records that actually exist. Matched by the PR's raw
  // full local timestamp (not just the date), needed for matchStravaActivity's
  // time-proximity match.
  const matchable = [
    [longestRun, longestRunPr?.activityStartDateTimeLocalFormatted],
    ...Object.entries(paceRecords).map(([key, rec]) => [rec, byType[RUN_TIME_PR_TYPE_IDS[key]]?.activityStartDateTimeLocalFormatted]),
  ]
  for (const [rec, startDateTime] of matchable) {
    if (rec) rec.stravaActivityId = await matchStravaActivity(startDateTime)
  }
