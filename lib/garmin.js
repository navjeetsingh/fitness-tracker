// Pulls sleep, HRV, Body Battery, RHR, stress from Garmin Connect and caches the
// result in Redis. Garmin Connect has no public API — this uses an unofficial,
// reverse-engineered client, so a fresh login on every page view risks the account
// getting flagged. Every dashboard visit reads the cache; only a stale cache
// triggers a real login (same caching approach as lib/strava.js's access token).
